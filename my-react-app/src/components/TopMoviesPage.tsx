...
const TopMoviesPage: React.FC<TopMoviesPageProps> = ({ apiKey }) => {
  ...
  const fetchTopMovies = async () => {
    try {
      const response = await fetch(
        `https://api.themoviedb.org/3/movie/top_rated?api_key=${apiKey}&language=ru-RU&page=${page}`
      );
        ...
        {movies.map((movie, index) => (
          ...
          >
            <div className="movie-poster">
              ...
              <div className="rank-badge">{(page - 1) * 20 + index + 1}</div>
            </div>
